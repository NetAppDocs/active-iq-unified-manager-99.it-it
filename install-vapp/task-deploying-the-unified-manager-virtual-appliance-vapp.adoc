...
A partire da Unified Manager 9.8, VMware Tools è stato sostituito con Open VM Tools  `open-vm-tools`). Non è necessario installare VMware Tools come parte del processo di installazione perché `open-vm-tools` È incluso nel pacchetto di installazione di Unified Manager.
...
È possibile accedere all'interfaccia utente Web per eseguire la configurazione iniziale di Unified Manager, come descritto in link:../config/task-using-the-maintenance-console.html["Utilizzando la console di manutenzione"].
